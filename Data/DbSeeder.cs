    ...
    public static async Task SeedDefaultData(IServiceProvider service)
    {
        try
        {
            ...
            var context = service.GetService<ApplicationDbContext>()
                ?? throw new InvalidOperationException("ApplicationDbContext not found in services");

            // Ensure database is created and all tables exist
            await context.Database.EnsureCreatedAsync();

            // Manually create Wishlist table if it doesn't exist
            ...
            // Manually create Review table if it doesn't exist
            await context.Database.ExecuteSqlRawAsync(@"
            ...
            var userMgr = service.GetService<UserManager<IdentityUser>>()
                ?? throw new InvalidOperationException("UserManager not found in services");
            var roleMgr = service.GetService<RoleManager<IdentityRole>>()
                ?? throw new InvalidOperationException("RoleManager not found in services");
            ...
            if (admin != null && !await userMgr.IsInRoleAsync(admin, Roles.Admin.ToString()))
            {
